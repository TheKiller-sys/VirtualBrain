...
async function initializeDatabase() {
    console.log('🧠 Inicializando Base de Datos Ultra Avanzada...');
    console.log('📊 Creando estructura completa del cerebro digital');
    ...
    if (initialized) {
        ...
        if (tables.length > 10) console.log(`   ... y ${tables.length - 10} tablas más`);
...
async function insertSampleData(db) {
    // 1. Estados iniciales del sistema
    const states = [
    ...
    for (const sample of bioSamples) {
        await db.saveBiochemicalState(sample);
    }
    
    // 3. Muestras emocionales
    const emoSamples = [
    ...
    for (const sample of emoSamples) {
        await db.saveEmotionalState(sample);
    }
    
    // 4. Muestras cognitivas
    const cogSamples = [
    ...
    for (const sample of cogSamples) {
        await db.saveCognitiveState(sample);
    }
    
    // 5. Muestras de personalidad
    ...
    for (const sample of personalitySamples) {
        await db.savePersonality(sample);
    }
    
    // 6. Memorias de ejemplo
    const memories = [
    ...
    // 7. Decisiones de ejemplo
    const decisions = [
        { decision: 'Explorar entorno', confianza: 0.8, contexto: 'situación nueva', resultado: 'exito', emocion_dominante: 'curiosidad' },
        { decision: 'Analizar situación', confianza: 0.9, contexto: 'evaluación de riesgos', resultado: 'exito', emocion_dominante: 'confianza' },
        { decision: 'Buscar ayuda', confianza: 0.7, contexto: 'situación compleja', resultado: 'pendiente', emocion_dominante: 'confianza' }
    ];
    ...
    const thoughts = [
        ...
        { contenido: 'Hay patrones emergentes', tipo: 'subconsciente', intensidad: 0.5, emocion_asociada: 'intuicion' }
    ];
    
    for (const thought of thoughts) {
        await db.saveThought(thought);
    }
    
    // 9. Conexiones neuronales de ejemplo
    const connections = [
    ...
    for (const conn of connections) {
        await db.saveConnection(conn.origin, conn.destination, conn.strength);
    }
    
    // 10. Metas de motivación
    const goals = [
        { tipo: 'aprendizaje', descripcion: 'Explorar nuevos conceptos', impulso_asociado: 'curiosidad', prioridad: 8, progreso: 30 },
        { tipo: 'crecimiento', descripcion: 'Desarrollar habilidades', impulso_asociado: 'logro', prioridad: 7, progreso: 50 },
        { tipo: 'social', descripcion: 'Conectar con otros', impulso_asociado: 'afiliacion', prioridad: 6, progreso: 20 }
    ];
    ...
    for (const goal of goals) {
        await db.db.run(`
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `, [Date.now(), goal.tipo, goal.descripcion, goal.impulso_asociado, goal.prioridad, goal.progreso, 0]);
    }
    ...
    const patterns = [
        ...
        { tipo: 'emocional', patron: 'espiral_alegria', confianza: 0.8, frecuencia: 2 },
        { tipo: 'cognitivo', patron: 'analisis_profundo', confianza: 0.7, frecuencia: 6 }
    ];
    ...
    for (const pattern of patterns) {
        await db.db.run(`
            ...
            VALUES (?, ?, ?, ?, ?)
        `, [Date.now(), pattern.tipo, pattern.patron, pattern.confianza, pattern.frecuencia]);
    }
    ...
    for (const skill of skills) {
        await db.db.run(`
            INSERT INTO motor_habilidades (nombre, tipo, complejidad, nivel, practicas, eficiencia, mastery, ultimo_uso)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `, [skill.nombre, skill.tipo, skill.complejidad, skill.nivel, skill.practicas, skill.eficiencia, skill.mastery, Date.now()]);
    }
